...
if [ "$USE_CUDA" = 'ON' ]; then
  ...
  CMAKE_FLAGS="-DGPU_TARGETS=${CMAKE_HIP_ARCHITECTURES} -DCMAKE_HIP_ARCHITECTURES=${CMAKE_HIP_ARCHITECTURES} -DUSE_HIP=$USE_HIP -DEXTERNAL_DMLC_LIB_PATH=$EXTERNAL_DMLC_LIB_PATH"
fi
