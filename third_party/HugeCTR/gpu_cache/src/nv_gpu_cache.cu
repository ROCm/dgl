...
__forceinline__ __device__ long long atomicAdd(
...
__forceinline__ __device__ unsigned long atomicAdd(
  ...
  return (unsigned long)atomicAdd(
      (unsigned long long*)address, (unsigned long long)val);
}
#endif
...
__forceinline__ __device__ void warp_tile_copy(
    const size_t lane_idx, const size_t emb_vec_size_in_float, float* d_dst,
    const float* d_src) {
#pragma unroll
...
template <int warp_size>
__forceinline__ __device__ void warp_tile_copy(
    const size_t lane_idx, const size_t emb_vec_size_in_float,
...
// Expect only one thread_block_tile return to execute critical section at any
// time
template <typename mutex, int warp_size>
__forceinline__ __device__ void warp_lock_mutex(
    const cg::thread_block_tile<warp_size>& warp_tile, mutex& set_mutex) {
  // The first thread of this (sub-)warp to acquire the lock
  if (warp_tile.thread_rank() == 0) {
    set_mutex.acquire();
  }
  warp_tile.sync();  // Synchronize the threads in the (sub-)warp. Execution
                     // barrier + memory fence
}

// The (sub-)warp holding the mutex will unlock the mutex after finishing the
// critical section on a set Expect any following (sub-)warp that acquire the
// mutex can see its modification done in the critical section
template <typename mutex, int warp_size>
__forceinline__ __device__ void warp_unlock_mutex(
    const cg::thread_block_tile<warp_size>& warp_tile, mutex& set_mutex) {
  warp_tile.sync();  // Synchronize the threads in the (sub-)warp. Execution
                     // barrier + memory fence
...
template <int warp_size>
__forceinline__ __device__ void warp_lock_mutex(
    const cg::thread_block_tile<warp_size>& warp_tile,
...
__forceinline__ __device__ void warp_min_reduction(
    const cg::thread_block_tile<warp_size>& warp_tile,
    ref_counter_type& min_slot_counter_val, size_t& slab_distance,
  ...
  for (size_t i = (warp_tile.size() >> 1); i > 0; i = i >> 1) {
    ref_counter_type input_slot_counter_val =
        warp_tile.shfl_xor(min_slot_counter_val, (int)i);
...
template <
    typename slabset, typename ref_counter_type,
    typename atomic_ref_counter_type, typename key_type, typename mutex>
__global__ void init_cache(
    slabset* keys, ref_counter_type* slot_counter,
    atomic_ref_counter_type* global_counter, const size_t num_slot,
    const key_type empty_key, mutex* set_mutex, const size_t capacity_in_set) {
...
__global__ void destruct_kernel(
    atomic_ref_counter_type* global_counter, mutex* set_mutex,
...
__global__ void update_kernel_overflow_ignore(
    atomic_ref_counter_type* global_counter, size_t* d_missing_len) {
...
__global__ void update_kernel_overflow_ignore(
    ref_counter_type* global_counter, size_t* d_missing_len) {
  // Update global counter
...
template <
    typename key_type, typename ref_counter_type,
    typename atomic_ref_counter_type, typename slabset, typename set_hasher,
    typename slab_hasher, typename mutex, key_type empty_key,
    int set_associativity, int warp_size>
__global__ void get_kernel(
  ...
  while (active_mask != 0) {
...
#ifdef __HIPCC__
    int next_lane = __ffs((unsigned int)active_mask) - 1;
#else
    ...
    while (active_mask == old_active_mask) {
      // When all the slabs inside a slabset have been searched, mark missing
      ...
      if (found_lane >= 0) {
        ...
        if (lane_idx == (size_t)next_lane) {
          slot_counter[found_offset] =
              global_counter->load(cuda::std::memory_order_relaxed);
          active = false;
        }
      ...
      // Compare the slab data with empty key, if found empty key, mark missing
      // task, task is completed
      ...
      // Not found in this slab, the task is not completed, goto searching next
      // slab
      counter++;
...
// Also update locality information for touched slot
template <
    typename key_type, typename ref_counter_type, typename slabset,
    typename set_hasher, typename slab_hasher, key_type empty_key,
    int set_associativity, int warp_size>
__global__ void get_kernel(
    const key_type* d_keys, const size_t len, float* d_values,
    const size_t embedding_vec_size, uint64_t* d_missing_index,
    key_type* d_missing_keys, size_t* d_missing_len,
    ref_counter_type* global_counter, volatile ref_counter_type* slot_counter,
    const size_t capacity_in_set, volatile slabset* keys, volatile float* vals,
...
template <
    typename key_type, typename slabset, typename ref_counter_type,
    typename mutex, typename atomic_ref_counter_type, typename set_hasher,
    typename slab_hasher, key_type empty_key, int set_associativity,
    int warp_size,
    ref_counter_type max_ref_counter_type =
        std::numeric_limits<ref_counter_type>::max(),
    size_t max_slab_distance = std::numeric_limits<size_t>::max()>
__global__ void insert_replace_kernel(
    const key_type* d_keys, const float* d_values,
    const size_t embedding_vec_size, const size_t len, slabset* keys,
    float* vals, ref_counter_type* slot_counter, mutex* set_mutex,
    const atomic_ref_counter_type* global_counter, const size_t capacity_in_set,
    const size_t task_per_warp_tile) {
  // Lane(thread) ID within a warp_tile
  cg::thread_block_tile<warp_size> warp_tile =
      cg::tiled_partition<warp_size>(cg::this_thread_block());
  ...
  while (active_mask != 0) {
    // Next task in the work quere, start from lower index lane(thread)
    int next_lane = __ffs(active_mask) - 1;
    // Broadcast the task, the global index and the src slabset and slab to all
    // lane in a warp_tile
    key_type next_key = warp_tile.shfl(key, next_lane);
    size_t next_idx = warp_tile.shfl(key_idx, next_lane);
    size_t next_set = warp_tile.shfl(src_set, next_lane);
    ...
    // Variable to keep the slot distance for slot with min counter within the
    // slab
    size_t slot_distance;
    // Working queue before task started
    const unsigned old_active_mask = active_mask;
    ...
    while (active_mask == old_active_mask) {
      ...
      if (counter >= set_associativity) {
        ...
        warp_min_reduction<ref_counter_type, warp_size>(
            warp_tile, min_slot_counter_val, slab_distance, slot_distance);

        // Calculate the position of LR slot
        size_t target_slab = (first_slab + slab_distance) % set_associativity;
        size_t slot_index =
            (next_set * set_associativity + target_slab) * warp_size +
            slot_distance;

        // Replace the LR slot
        if (lane_idx == (size_t)next_lane) {
          keys[next_set].set_[target_slab].slab_[slot_distance] = key;
          slot_counter[slot_index] =
              global_counter->load(cuda::std::memory_order_relaxed);
        }
      ...
      ref_counter_type read_slot_counter = slot_counter
          [(next_set * set_associativity + next_slab) * warp_size + lane_idx];
...
__global__ void insert_replace_kernel(
    const key_type* d_keys, const float* d_values,
    const size_t embedding_vec_size, const size_t len, volatile slabset* keys,
    volatile float* vals, volatile ref_counter_type* slot_counter,
    volatile int* set_mutex, ref_counter_type* global_counter,
    const size_t capacity_in_set, const size_t task_per_warp_tile) {
  // Lane(thread) ID within a warp_tile
  cg::thread_block_tile<warp_size> warp_tile =
      cg::tiled_partition<warp_size>(cg::this_thread_block());
  const size_t lane_idx = warp_tile.thread_rank();
...
template <
    typename key_type, typename slabset, typename set_hasher,
    typename slab_hasher, typename mutex, key_type empty_key,
    int set_associativity, int warp_size>
__global__ void update_kernel(
    const key_type* d_keys, const size_t len, const float* d_values,
  ...
  while (active_mask != 0) {
    ...
    while (active_mask == old_active_mask) {
      ...
      // Compare the slab data with empty key, if found empty key, mark missing
      // task, do nothing, task is completed
      if (warp_tile.ballot(read_key == empty_key) != 0) {
        if (lane_idx == (size_t)next_lane) {
          active = false;
        }
...
#ifdef LIBCUDACXX_VERSION
template <
    typename key_type, typename slabset, typename mutex, key_type empty_key,
    int set_associativity, int warp_size>
__global__ void dump_kernel(
    key_type* d_keys, size_t* d_dump_counter, const slabset* keys,
  ...
  if (set_idx < end_set_index) {
    ...
    // Each lane(thread) within the warp tile calculate the offset to store its
    // keys
  ...
  if (set_idx < end_set_index) {
    for (unsigned slab_id = 0; slab_id < set_associativity; slab_id++) {
      if (read_key[slab_id] != empty_key) {
        d_keys
            [block_key_offset + warp_key_offset + thread_key_offset[slab_id]] =
                read_key[slab_id];
      }
    }
  }
...
#else
template <
    typename key_type, typename slabset, key_type empty_key,
    int set_associativity, int warp_size>
__global__ void dump_kernel(
    key_type* d_keys, size_t* d_dump_counter, volatile slabset* keys,
  ...
  if (set_idx < end_set_index) {
    ...
    for (unsigned slab_id = 0; slab_id < set_associativity; slab_id++) {
      // The warp tile read out a slab
      read_key[slab_id] =
          ((volatile key_type*)(keys[set_idx].set_[slab_id].slab_))[lane_idx];
    }
...
#ifdef LIBCUDACXX_VERSION
template <
    typename key_type, typename ref_counter_type, key_type empty_key,
    int set_associativity, int warp_size, typename set_hasher,
    typename slab_hasher>
gpu_cache<
    key_type, ref_counter_type, empty_key, set_associativity, warp_size,
    set_hasher, slab_hasher>::
  ...
  if (warp_size != 1 && warp_size != 2 && warp_size != 4 && warp_size != 8 &&
  ...
  CUDA_CHECK(cudaMalloc(
      (void**)&vals_, sizeof(float) * embedding_vec_size_ * num_slot_));
  CUDA_CHECK(
      cudaMalloc((void**)&slot_counter_, sizeof(ref_counter_type) * num_slot_));
  CUDA_CHECK(
      cudaMalloc((void**)&global_counter_, sizeof(atomic_ref_counter_type)));
  ...
  init_cache<<<((num_slot_ - 1) / BLOCK_SIZE_) + 1, BLOCK_SIZE_>>>(
      keys_, slot_counter_, global_counter_, num_slot_, empty_key, set_mutex_,
      capacity_in_set_);
...
gpu_cache<
    key_type, ref_counter_type, empty_key, set_associativity, warp_size,
    set_hasher, slab_hasher>::~gpu_cache() {
  // Device Restorer
...
gpu_cache<
    key_type, ref_counter_type, empty_key, set_associativity, warp_size,
    set_hasher, slab_hasher>::~gpu_cache() noexcept(false) {
  // Device Restorer
  nv::CudaDeviceRestorer dev_restorer;
...
void gpu_cache<
  ...
  // Update the global counter as user perform a new(most recent) read operation
  // to the cache Resolve distance overflow issue as well.
  ...
  get_kernel<
      key_type, ref_counter_type, atomic_ref_counter_type, slabset, set_hasher,
      slab_hasher, mutex, empty_key, set_associativity, warp_size>
      <<<grid_size, BLOCK_SIZE_, 0, stream>>>(
          d_keys, len, d_values, embedding_vec_size_, d_missing_index,
          d_missing_keys, d_missing_len, global_counter_, slot_counter_,
          capacity_in_set_, keys_, vals_, set_mutex_, task_per_warp_tile);
...
void gpu_cache<
  ...
  const size_t grid_size = ((len - 1) / keys_per_block) + 1;
  get_kernel<
      key_type, ref_counter_type, slabset, set_hasher, slab_hasher, empty_key,
...
void gpu_cache<
  ...
  // Try to insert the <k,v> paris into the cache as long as there are unused
  // slot Then replace the <k,v> pairs into the cache
  const size_t keys_per_block = (BLOCK_SIZE_ / warp_size) * task_per_warp_tile;
  const size_t grid_size = ((len - 1) / keys_per_block) + 1;
  insert_replace_kernel<
      key_type, slabset, ref_counter_type, mutex, atomic_ref_counter_type,
      set_hasher, slab_hasher, empty_key, set_associativity, warp_size>
      <<<grid_size, BLOCK_SIZE_, 0, stream>>>(
          d_keys, d_values, embedding_vec_size_, len, keys_, vals_,
          slot_counter_, set_mutex_, global_counter_, capacity_in_set_,
          task_per_warp_tile);
...
void gpu_cache<
  ...
  const size_t grid_size = ((len - 1) / keys_per_block) + 1;
  insert_replace_kernel<
      key_type, slabset, ref_counter_type, set_hasher, slab_hasher, empty_key,
      set_associativity, warp_size><<<grid_size, BLOCK_SIZE_, 0, stream>>>(
      d_keys, d_values, embedding_vec_size_, len, keys_, vals_, slot_counter_,
      set_mutex_, global_counter_, capacity_in_set_, task_per_warp_tile);
...
void gpu_cache<
  ...
  const size_t grid_size = ((len - 1) / keys_per_block) + 1;
  update_kernel<
      key_type, slabset, set_hasher, slab_hasher, mutex, empty_key,
      set_associativity, warp_size><<<grid_size, BLOCK_SIZE_, 0, stream>>>(
...
void gpu_cache<
  ...
  const size_t grid_size = ((len - 1) / keys_per_block) + 1;
  update_kernel<
      key_type, slabset, set_hasher, slab_hasher, empty_key, set_associativity,
...
void gpu_cache<
    ...
    set_hasher, slab_hasher>::
    Dump(
        key_type* d_keys, size_t* d_dump_counter, const size_t start_set_index,
  ...
  dump_kernel<key_type, slabset, mutex, empty_key, set_associativity, warp_size>
      <<<grid_size, BLOCK_SIZE_, 0, stream>>>(
          d_keys, d_dump_counter, keys_, set_mutex_, start_set_index,
          end_set_index);
...
template class gpu_cache<
    unsigned int, uint64_t, std::numeric_limits<unsigned int>::max(),
    SET_ASSOCIATIVITY, SLAB_SIZE>;
template class gpu_cache<
    long long, uint64_t, std::numeric_limits<long long>::max(),
    SET_ASSOCIATIVITY, SLAB_SIZE>;
}  // namespace gpu_cache
